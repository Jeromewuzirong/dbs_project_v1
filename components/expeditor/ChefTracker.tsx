...
interface Props {
  ...
  unassignedTask: ChefTask | null;
}

export default function ChefTracker({ chefs, chefTasks, unassignedTask }: Props) {
  ...
  return (
    <div className="border-t border-gray-800 p-4 flex flex-col gap-3">
      ...
      {sorted.length === 0 && !unassignedTask ? (
        ...
        <>
          ...
          {unassignedTask && (
            <div className="bg-gray-900 border border-gray-700 border-dashed rounded-lg p-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-semibold text-gray-400 italic truncate mr-2">
                  Unassigned
                </span>
                <span className="text-xs font-medium shrink-0 text-amber-400">
                  T{unassignedTask.tableNumber}
                </span>
              </div>
              <p className="text-xs text-white leading-tight">
                Table {unassignedTask.tableNumber} · {unassignedTask.dishName} · {unassignedTask.stepName}
              </p>
            </div>
          )}
        </>
